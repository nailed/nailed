...
object CommandHelp extends Command("help", "?") {
  ...
  override def execute(ctx: CommandContext, args: Arguments){
    ...
    val page = args.getInt(0, 1, pages + 1, 1) - 1

    //TODO: /help commandName - give help about the command

    ctx.sendMessage(new ComponentBuilder("--- Showing help page %d of %d (/help <page>) ---".format(page + 1, pages + 1)).color(ChatColor.DARK_GREEN).create())
    ...
    for(i <- page * 7 until limit){
      ...
      ctx.sendMessage(comp)
    }

    if(page == 0){
      ctx.sendMessage(new ComponentBuilder("Tip: Use the <tab> key while typing a command to auto-complete the command or its arguments").color(ChatColor.GREEN).create())
    }
  }
}
